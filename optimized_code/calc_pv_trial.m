...
function [SigmaCtllhInv, wt, Psit, zc] = calc_pv_trial(x, v, y, Z, sigmaU, sigmaV, phi)
    ...
    sum_phi = sum(phi, 2);
    sum_phi_scaled = 1 + sum_phi;

    A = phi ./ sum_phi_scaled;
    sSq = sigmaV^2 ./ sum_phi_scaled;

    ptAct = find(any(ZAct .* x, 1)); 
    ptNo = length(ptAct);
    ...
    if ptNo > 0
        ...
        sSq_act = sSq(xAct);
        weights = 1 ./ sSq_act;
        SigmaInv = zeros(ptNo, ptNo, xActNo);

        for xi = 1:xActNo
            a = A_act(xi, :)';
            SigmaInv(:,:,xi) = (a * a') * weights(xi);
        end

        SigmaCVInv = sum(SigmaInv, 3);
        SigmaCInv = SigmaCVInv + eye(ptNo)/sigmaU^2;
        SigmaC = SigmaCInv \ eye(ptNo);
        logDetSigmaC = log(det(SigmaC));

        temp = A_act * (SigmaC * A_act');
        omega = (1/sigmaV^4) * (diag(sSq_act) - temp);
        omegaBU = zeros(N);
        omegaBU(xAct, xAct) = omega;

        rows = availLinks(:, 1);
        cols = availLinks(:, 2);
        linearInd = sub2ind(size(phi), rows, cols);
        phi_avail = phi(linearInd);

        SigmaCtllhInv = phi_avail .* (omegaBU(rows, rows) .* phi_avail');
        ...
        for comp = 1:2
            vcomp = vnan(:, :, comp);
            weighted_v = sum_phi_scaled .* vcomp;
            wt(:, 1, comp) = phi_avail .* (omegaBU(rows, :) * weighted_v);
        end
    else
        omegaBU = zeros(N);
    end
    ...
    sSq_act = sSq(xAct);
    first_term = -xActNo/2 * log(2*pi) ... 
                     - ptNo*log(sigmaU) ...
                     - 0.5*sum(log(sSq_act)) ...
                     + 0.5*logDetSigmaC;

    second_term = exp(first_term);

    Psit = zeros(1, 1, 2);
    for comp = 1:2
        vcomp = v(:, :, comp);
        vcomp(isnan(vcomp)) = 0;
        vp = sum_phi_scaled .* vcomp;
        exponent = -0.5 * (vp' * omegaBU * vp);
        Psit(1, 1, comp) = second_term * exp(exponent);
    end
end
%%