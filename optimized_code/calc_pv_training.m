...
%%%%%%%%%%%%%%%%% calc_pv_training with cache &&&&&&&&&&&&&&&&
function [p, muCTpost, SigmaCTpostInv, SigmaCTpost, zc, Psit] = ...
    calc_pv_training(x,v,y,Z, sigmaU, sigmaV, phi, sigmaC, varargin)

    persistent GLOBAL_CACHE;
    if isempty(GLOBAL_CACHE)
        GLOBAL_CACHE = containers.Map();
    end
    ...
    % Cache based on y and Z
    %y_hash = DataHash(y);
    %Z_hash = DataHash(Z);
    %GetMD5 is faster than DataHash but only by 5 sec. for T=144
    y_hash = GetMD5(y);
    Z_hash = GetMD5(Z);

    cache_key = [y_hash, '_', Z_hash];

    if isKey(GLOBAL_CACHE, cache_key)
        cached_result = GLOBAL_CACHE(cache_key);
        p = cached_result.p;
        muCTpost = cached_result.muCTpost;
        SigmaCTpostInv = cached_result.SigmaCTpostInv;
        SigmaCTpost = cached_result.SigmaCTpost;
        zc = cached_result.zc;
        Psit = cached_result.Psit;
        return;
    end

    %% I added the Vectorized part to speed up the computation
    [N, T] = size(x);
    K = size(Z,2);
    availLinkNo = sum(Z(:));
    zc = false(N, K);
    ...
    for t = 1:T
       [SigmaCtllhInv(:,:,t), wt(:,t,:), Psit(:,t,:), zct] = ...
            calc_pv_trial_mex_jit(x(:,t), v(:,t,:), y(:,t), Z, sigmaU, sigmaV, phi);
        zc = (zc|zct);
    end

    %% Vectorized
    SigmaCTllhInv = sum(SigmaCtllhInv, 3);                            
    SigmaCTpostInv = SigmaCTllhInv;
    diag_idx = 1:availLinkNo+1:availLinkNo*availLinkNo;
    SigmaCTpostInv(diag_idx) = SigmaCTpostInv(diag_idx) + 1/(sigmaC^2);
    %%

    SigmaCTpost = inv(SigmaCTpostInv);
    muCTpost = SigmaCTpost * sum(wt(:,:,1), 2);
    muCTpost(:,:,2) = SigmaCTpost * sum(wt(:,:,2), 2);
    ...
    result.p = p;
    result.muCTpost = muCTpost;
    result.SigmaCTpostInv = SigmaCTpostInv;
    result.SigmaCTpost = SigmaCTpost;
    result.zc = zc;
    result.Psit = Psit;
