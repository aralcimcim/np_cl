function [Znew Ynew Rnew ] = wood_make_gibbs_z_spatial_NS(Y,X,V,Z,R, lda, eps, sigmaU, sigmaV, phi, sigmaC, Balpha, Bbeta, alpha,Kmax, wburn, wsample)
%rng(0);
...
for n = 1:N, %** for each shapes
    ...
    Znew = Z;
 %VAL      lpv_y_new = calc_pv_training(X, V, Y, Znew, sigmaU, sigmaV, phi, sigmaC, 0, 1);  %** p(v | sigma_v) EQ 57 OW
 ...
 %VAL              lpv_y(1, 1+Znew(n,k)) = lpv_y_new;%calc_pv_training(X, V, Y, ZAct, sigmaU, sigmaV, phi, sigmaC,0,1); %lpv_y_new;
            lpx_y(1, 1+Znew(n,k)) = lpx_y_new;%calc_px_training(X(n,:), Y, ZAct(n,:), lda, eps, 1); 
           
            %** z_nk = 1/0, the opposite of Z (in EQ 12 OW)
            Znew(n, k) = 1 - Znew(n, k);
 %VAL              lpv_y(1, 1 + Znew(n,k)) = calc_pv_training(X,V,Y,Znew, sigmaU, sigmaV, phi, sigmaC,0,1); 
            lpx_y(1, 1 + Znew(n,k)) = calc_px_training(X(n,:),Y, Znew(n,:), lda, eps, 1);
            
            %** EQ 12 OW
            lPznk1_XZmnkY = log(th_k) + lpx_y(1, 2) ; %VAL + lpv_y(1, 2); 
            lPznk0_XZmnkY = log(1 - th_k) + lpx_y(1, 1); %VAL  + lpv_y(1, 1);
            %** EQ 13 OW
            ...
            if (rt == Inf) 
                 Pznk_XZmnkY = 1;
            else Pznk_XZmnkY = rt/(rt+1); %** similarly to y_spatial, this is P1/(P1+P0) in EQ 13 (without normalizing the probabilities), EQ 13 OW
            end;
 ...
 %VAL          lpv_y_new = lpv_y(1, Znew(n,k) + 1);
...
%VAL           lpv_y_new = calc_pv_training(X, V, YAct, ZAct, sigmaU, sigmaV, phi, sigmaC, 0, 1);
   ...
   %VAL                        lpv_y_act(1, 1 + YAct(K+k, t)) =  lpv_y_new;
                        lpx_y_act(1, 1 + YAct(K+k, t)) =  lpx_y_new;
                        
                        YAct(K+k, t) = 1 - YAct(K+k, t);
   %VAL                        lpv_y_act(1, 1 + YAct(K+k, t)) = calc_pv_training(X,V,YAct,ZAct, sigmaU, sigmaV, phi, sigmaC,0,1);%...
                        lpx_y_act(1, 1 + YAct(K+k, t)) = calc_px_training(X(n,:), YAct, ZAct(n,:), lda, eps, 1);  %** CHANGED from calc_px_training(X,YAct,ZAct, lda, eps, 1);
                        
                        p = RAct(1, K + k);
                        %YAct(K+k,t)=1;
                        lPYk1_ZXYmk = log(p) + lpx_y_act(1,2) ;%VAL   + lpv_y_act(1,2);
                        %YAct(K+k,t)=0;
                        lPYk0_ZXYmk = log(1-p) + lpx_y_act(1,1);%VAL    + lpv_y_act(1,1);

                        rt = exp(lPYk1_ZXYmk - lPYk0_ZXYmk );
                        
                        if (rt == Inf) 
                            PYk1_ZXYmk = 1;
                        else PYk1_ZXYmk = rt / (rt+1); 
                        end;
  ...
  %VAL                         lpv_y_new = lpv_y_act(1, YAct(K + k, t) + 1);
 ...
 %VAL                      lpy_v(wi-wburn) = lpv_y_new;
                    lpy_x(wi-wburn) = lpx_y_new;   
                    lpy_xv(wi-wburn) = lpx_y_new ;%VAL   + lpv_y_new ;
                end
 ...
 %VAL              lpv_y_new = calc_pv_training(X,V,YAct,ZAct, sigmaU, sigmaV, phi, sigmaC,0,1);
            ...
            lpy_xv = lpx_y_new; %VAL  +lpv_y_new  ;                   
