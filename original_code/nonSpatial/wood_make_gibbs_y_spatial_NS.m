function Ynew = wood_make_gibbs_y_spatial_NS(Y,X,V,Z,R, lda, eps, sigmaU, sigmaV, phi, sigmaC)
    %rng(0);
 ...
 %VAL         lpv_y_new = calc_pv_training(X, V, Ynew, Z, sigmaU, sigmaV, phi, sigmaC,0,1);
  ...
  %VAL                lpv_y(1, 1 + Ynew(k,t)) = lpv_y_new;
  ...
  %VAL                lpv_y(1, 1 + Ynew(k,t)) = calc_pv_training(X, V, Ynew, Z, sigmaU, sigmaV, phi, sigmaC,0,1);  %calc_pv_training(X,V,Ynew,Z, sigmaU, sigmaV, phi, sigmaC,0,1); 
            ...
            %** p(y = 1 | ...)
            lPYk1_ZXYmk = log(p) + lpx_y(1, 2) ; %VAL     + lpv_y(1, 2);

            %** p(y = 0 | ...)
            lPYk0_ZXYmk = log(1 - p) + lpx_y(1, 1) ; %VAL     + lpv_y(1, 1);
  ...
  %VAL                lpv_y_new = lpv_y(1, Ynew(k, t) + 1);
